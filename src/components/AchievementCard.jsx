...
const AchievementCard = ({ achievement, dark }) => {
    ...
    return (
        <div 
            ...
            data-aos-delay="200"
            className={`relative group ${dark ? 'bg-slate-900 shadow-stone-600' : 'bg-slate-300 ackdrop-blur-2xl '} h-max shadow-lg md:ml-4   w-full pt-10 pb-8 px-3 md:px-6 cursor-pointer rounded-2xl  transition-all duration-300 overflow-hidden`}>
            ...
            <div className='w-full flex flex-col gap-y-2 justify-between'>
                <div className={`text-[1.2rem] font-semibold ${dark ? 'text-pink-600' : 'text-sky-900'} tracking-wider `}>{title}</div>
            </div>
        </div>
    )
}
